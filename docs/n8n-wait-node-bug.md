...
For **Connection Sync → HubSpot** (workflow 3): when an execution fails after processing some profiles (e.g. at the Wait node), do not rely on resuming the same execution. Instead:

1. Run `./scripts/n8n-ops/debug.sh full <execution_id>` (with `N8N_API_KEY` set) to save execution data.
2. Run `./scripts/n8n-ops/debug.sh resume <execution_id> --exclude-completed` to extract the remaining profiles and POST them to the Test Webhook: From Merge (`/webhook/test-from-merge`). The workflow will process the remaining items in a new run.
