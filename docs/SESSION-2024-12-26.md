...
- `scripts/n8n-ops/debug.sh` - Added execution data functions
...
1. Pipeline Monitor Execution #397 errored at "Dedupe Against Master + New Leads"
   ...
   - Debug command: `./scripts/legacy/auto-debug.sh analyze 397`
...
- [ ] figure out tools to make sure that it uses the correct paths when creating new workflows to ensure that its either updating existing workflows or only creating new ones when they don't exist. Maybe it creates the same version of the github chrt sync workflow locally or uses it each time to ensure that everything flows correctly. Have seen multiple workflows created in the personal folder when should be in the chrt project 
...
### Debug an Error
```bash
./scripts/legacy/auto-debug.sh check        # Find errors
./scripts/legacy/auto-debug.sh analyze 397  # Analyze specific execution
```
